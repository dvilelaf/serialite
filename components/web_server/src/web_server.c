...
static esp_err_t terminal_handler(httpd_req_t *req)
{
    ...
    const int written = snprintf(
        ...
        ".pill{border:1px solid var(--line);border-radius:999px;padding:5px 9px;color:var(--muted);background:#050907;font-size:12px;letter-spacing:.04em}.stream{display:flex;align-items:center;gap:7px;max-width:74px;overflow:hidden;white-space:nowrap;transition:max-width .16s ease,color .16s ease}.stream.expanded,.stream:hover,.stream:focus{max-width:min(360px,calc(100vw - 32px));color:var(--text)}.dot{flex:0 0 auto;width:9px;height:9px;border-radius:50%%;background:var(--warn);box-shadow:0 0 12px var(--warn)}.dot.ok{background:#43ff9b;box-shadow:0 0 14px #43ff9b}.dot.bad{background:var(--bad);box-shadow:0 0 14px var(--bad)}#streamText{overflow:hidden;text-overflow:ellipsis}.stream.expanded #streamText,.stream:hover #streamText,.stream:focus #streamText{text-overflow:clip}"
