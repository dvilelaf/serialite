...
static esp_err_t terminal_handler(httpd_req_t *req)
{
    ...
    const int written = snprintf(
        ...
        "let ws,backoff=500;function add(t){if(empty){terminal.textContent='';empty=false}terminal.textContent+=t;terminal.scrollTop=terminal.scrollHeight;if(terminal.textContent.length>65536)terminal.textContent=terminal.textContent.slice(-49152)}"
        "function connect(){state.textContent='STREAM ...';connected=false;render();const scheme=location.protocol==='https:'?'wss://':'ws://';ws=new WebSocket(scheme+location.host+'/ws');ws.binaryType='arraybuffer';"
        "ws.onopen=()=>{state.textContent='STREAM OK';connected=true;writerState='read-only';backoff=500;render();refreshStatus();terminal.focus()};"
        "ws.onmessage=e=>{if(e.data instanceof ArrayBuffer)add(new TextDecoder().decode(e.data));else add(e.data)};"
        "ws.onclose=()=>{connected=false;writerState='read-only';if(locked)return;state.textContent='RECONNECT';render();refreshStatus();setTimeout(connect,backoff);backoff=Math.min(backoff*2,5000)};"
        "ws.onerror=()=>ws.close()}"
        ...
        "function handleKey(e){const data=keyToData(e);if(data){send(data);e.preventDefault()}}"
        ...
        "terminal.addEventListener('keydown',handleKey);kbd.addEventListener('keydown',handleKey);kbd.addEventListener('input',()=>{if(kbd.value){send(kbd.value);kbd.value=''}});"
