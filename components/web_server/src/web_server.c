...
static esp_err_t terminal_handler(httpd_req_t *req)
{
    ...
    const int written = snprintf(
        ...
        "function setStream(text,ok){state.dataset.detail=text;state.title=text;streamText.textContent=ok?'OK':'ERROR';streamDot.className='dot '+(ok?'ok':'bad')}"
        ...
        "ws.onmessage=e=>{lastRx=Date.now();hideConsoleNotice();if(empty){term.clear();empty=false}if(e.data instanceof ArrayBuffer)term.write(new Uint8Array(e.data));else term.write(e.data)};"
        ...
        "document.getElementById('menuBtn').onclick=()=>document.getElementById('keys').classList.toggle('open');state.tabIndex=0;state.onclick=()=>state.classList.toggle('expanded');state.onmouseleave=()=>state.classList.remove('expanded');"
